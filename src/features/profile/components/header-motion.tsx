...
export const HeaderMotion = ({ isPlaying, currentTime, duration }: IProp) => {
  ...
  const { handlePauseAudio, handleResumeAudio } = useAudio();
  const progress = duration ? (currentTime / duration) * 100 : 0;

  return (
    <motion.header
      className="group fixed left-0 right-0 top-0 z-[1000] mx-auto flex h-16 w-fit justify-center bg-background pt-2 transition-[height] duration-300 ease-in-out hover:h-32"
      style={{ top }}
    >
        ...
        <div className="flex w-fit items-center justify-center space-x-2 rounded-full py-2 pl-2 pr-4 text-2xl font-bold">
          <>
            <div className="flex items-center justify-center">
              ...
              <svg
                className="ml-1 text-left text-3xl text-blue-600"
                width="0.6em"
                height="0.6em"
                viewBox="0 0 24 24"
                fill="none"
